...
pow <- group_by(dat, n, eff, maf, C, h2, model) %>%
	dplyr::summarise(
		pow1=sum(pval1 < 0.05) / n(), 
		pow2=sum(pval2 < 0.05) / n(), 
		pow3=sum(pval3 < 0.05) / n()
	)
...
pow <- gather(pow, key=method, value=pow, pow1, pow2, pow3)


assoc <- group_by(dat, n, eff, maf, C, h2, model) %>%
	dplyr::summarise(rel=cov(b1, b2)/var(b1))
...
ggplot(subset(pow, model == 1 & n == max(n)), aes(y=pow, x=eff)) +
