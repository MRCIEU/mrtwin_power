...
ggplot(subset(pow, model == 2 & n == max(n)), aes(y=pow, x=eff)) +
geom_point(aes(colour=as.factor(test))) +
geom_line(aes(colour=as.factor(test))) +
facet_grid(Clab ~ Modellab) +
labs(x="Effect of SNP on variance", y="Power (alpha = 5e-8)", colour="Test") +
...
ggplot(subset(pow, model == 3 & n == max(n)), aes(y=pow, x=eff)) +
geom_point(aes(colour=as.factor(test))) +
geom_line(aes(colour=as.factor(test))) +
facet_grid(Clab ~ Modellab) +
labs(x="Effect of SNP on variance", y="Power (alpha = 5e-8)", colour="Test") +
...
```{r, fig.cap="Power estimates for Model 1, where $\\alpha=5 \\times 10^{-8}$"}

ggplot(subset(pow, model == 1 & test == "D2"), aes(y=pow, x=eff)) +
