...
dat$b[dat$test %in% c("Hybrid", "Standard MR")] <- dat$b[dat$test %in% c("Hybrid", "Standard MR")]^2
...
pow2 <- subset(pow, test %in% c("Standard MR", "Hybrid"))
pow2$test <- as.factor(as.character(pow2$test))
levels(pow2$test) <- c("Within-family MR", "Standard MR")
...
ggplot(subset(pow, eff_xy == 0 & vargx == 0.1), aes(x=as.factor(n), y=pow, group=as.factor(test))) +
...
Plotting again with only hybrid and standard for purpose of cleaner presentations
